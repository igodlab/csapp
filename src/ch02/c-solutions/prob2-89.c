...
/* ── The five conditions (volatile prevents the compiler from collapsing   ── */
...
static int condC(double dx, double dy, double dz) {
     ...
     * bits), and the sum of any two int32 values fits in 33 bits, still ≪ 2^53.
...
static int condD(double dx, double dy, double dz) {
    /*
     * Products of two int32 values can be ~62 bits wide.  A double only holds
     * 53 bits of significand, so the product must be rounded.  The two
     * orderings round at different points → different final results.
     *
...
int main(void) {
    ...
    printf(DIM
           ...
           "    -mfpmath=sse -msse2 -fno-associative-math -fno-fma\n"
           RST);
